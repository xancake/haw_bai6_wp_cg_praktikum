...
public abstract class Scene extends JFrame {
	...
	public void init(GL2 gl) {
		...
		gl.glClearColor(
				(float) root.getBackgroundColor().x(),
				(float) root.getBackgroundColor().y(),
				(float) root.getBackgroundColor().z(),
				1.0f);
	...
	private void drawShadowVolume(GL2 gl) {
		...
		// Render the shadow volumes (because of culling, only their front faces
		// are
		// rendered).
	...
	private void drawDebugShadowVolume(GL2 gl) {
		...
		ShaderAttributes.getInstance().setShaderModeParameter(gl, root.getShader().getMode());
		getRoot().traverse(gl, RenderMode.DEBUG_SHADOW_VOLUME, Matrix.createIdentity4());
	...
	private void updateProjectionMatrix(GL2 gl) {
		if (needsUpdateProjectionMatrix) {
			...
			gl.glLoadIdentity();
			glu.gluPerspective(
					root.getCamera().getFovy(),
					root.getCamera().getAspectRatio(),
					root.getCamera().getZNear(),
	...
	private void updateViewMatrix(GL2 gl) {
		if (needsUpdateViewMatrix) {
			...
			glu.gluLookAt(
					cam.getEye().x(), cam.getEye().y(), cam.getEye().z(),
					cam.getRef().x(), cam.getRef().y(), cam.getRef().z(),
